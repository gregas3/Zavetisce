...
export default function QuickLinks() {
  return (
    <div className="container mx-auto px-4 py-8"> {/* Reduced padding from py-16 to py-8 */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {links.map((link, index) => (
          ...
          >
            <Link
              to={link.link}
              className="group p-6 md:p-8 rounded-xl border border-border bg-card flex flex-col h-full hover-lift overflow-hidden relative"
            >
              ...
              <div className="absolute bottom-0 right-0 w-24 h-24 -m-8 bg-accent rounded-full opacity-0 group-hover:opacity-30 transition-normal"></div>
            </Link>
          </AnimatedWrapper>
        ))}
      </div>
