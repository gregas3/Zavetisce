...
export default function SponsorsSection() {
  return <div className="py-4">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
        {sponsors.map(sponsor => <AnimatedWrapper key={sponsor.id} animation="zoom-in" delay={sponsor.delay} className="h-full">
            {/* For larger screens use HoverCard */}
            <div className="hidden md:block h-full">
              <HoverCard>
                <HoverCardTrigger asChild>
                  <a href={sponsor.website} target="_blank" rel="noopener noreferrer" className="block h-full" aria-label={`Obiščite spletno stran ${sponsor.name}`}>
                    ...
                    <a href={sponsor.website} target="_blank" rel="noopener noreferrer" className="block h-full" aria-label={`Obiščite spletno stran ${sponsor.name}`}>
                      <Card className="h-full transition-all duration-300 hover:shadow-md border border-teal-200 bg-white/80">
                        <CardContent className="p-3 flex flex-col items-center justify-center">
                          {/* Logo with image sprite technique */}
                          <div className="w-full h-16 mb-1 bg-contain bg-no-repeat bg-center" style={{
