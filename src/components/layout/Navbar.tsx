  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/80 backdrop-blur-md shadow-sm" : "bg-transparent"}`}>
      <div className="container mx-auto px-4 py-3">
        <div className="flex items-center justify-between">
          <Link to="/" className="text-2xl font-display font-bold text-primary flex items-center gap-2" onClick={closeMenu}>
            ...
            <img alt="Logo" className="h-10" src="/lovable-uploads/4538ce4d-c476-48c3-ad21-89aa38c7c769.png" />
          </Link>

          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center space-x-1">
            <NavLinks closeMenu={closeMenu} />
            <Button size="sm" asChild className="ml-4 bg-secondary hover:bg-secondary/90 text-secondary-foreground font-medium">
              <Link to="/donacije">Doniraj</Link>
            </Button>
          </nav>

          {/* Mobile Menu Button */}
          <button className="md:hidden text-primary p-2" onClick={toggleMenu} aria-label={isOpen ? "Zapri meni" : "Odpri meni"}>
      ...
      top: "60px"
        ...
        <nav className="container mx-auto px-6 py-8 flex flex-col space-y-6">
          ...
          <Button size="lg" asChild className="mt-6 bg-secondary hover:bg-secondary/90 text-secondary-foreground font-medium">
...
function NavLinks({
  ...
  const linkClass = isMobile ? "text-lg py-3 block font-medium border-b border-border" : "px-3 py-2 rounded-md text-sm font-medium hover:bg-accent transition-normal";
  return <>
